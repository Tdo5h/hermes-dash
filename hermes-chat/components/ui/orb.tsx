  ...
  useEffect(() => {
    ...
    const resetAfterResume = () => {
      ...
      raf = requestAnimationFrame(() => {
        raf = null
        setCanvasResetKey((v) => (v + 1) % 1000)
      })
    }

    const onVisibility = () => {
      if (document.visibilityState === "visible") resetAfterResume()
    }
...
function Scene({
  ...
  useEffect(() => {
    const apply = () => {
      ...
      requestAnimationFrame(() => {
        if (!circleRef.current) return
        circleRef.current.material.uniforms.uBgColor.value.copy(readBgColor())
      })
    }
  ...
  useFrame((_, delta: number) => {
    ...
    const isThinking = agentRef.current === "thinking"
    const isTyping = agentRef.current === "typing"
    const isEditing = agentRef.current === "editing"
    u.uTime.value += delta * (isThinking ? 0.75 : isTyping ? 0.68 : isEditing ? 0.62 : 0.5)
    ...
    if (modeRef.current === "manual") {
      ...
      } else if (agentRef.current === "typing") {
        targetIn = clamp01(0.72 + Math.sin(t * 5.4) * 0.2)
        targetOut = clamp01(0.82 + Math.sin(t * 4.4) * 0.2)
      } else if (agentRef.current === "editing") {
        targetIn = clamp01(0.58 + Math.sin(t * 3.6) * 0.3)
        targetOut = clamp01(0.5 + Math.sin(t * 2.8) * 0.22)
      } else if (agentRef.current === "thinking") {
        const pulse = 0.5 * (1 + Math.sin(t * 1.8))
        const base = 0.35 + 0.25 * pulse
        const wander = 0.12 * Math.sin(t * 3.6) * Math.sin(t * 1.1 + 1.4)
        targetIn = clamp01(base + wander)
        targetOut = clamp01(
          0.5 +
            0.30 * Math.sin(t * 2.1 + 0.3) +
            0.10 * Math.sin(t * 5.8 + 1.1)
        )
      } else {
    ...
    const baseSpeed = 0.1 + (1 - Math.pow(curOutRef.current - 1, 2)) * 0.9
    const thinkingBoost = isThinking
      ? 0.28 + 0.18 * Math.sin(u.uTime.value * 2.2)
      : isTyping
        ? 0.22 + 0.14 * Math.sin(u.uTime.value * 3.4)
        : isEditing
          ? 0.12 + 0.1 * Math.sin(u.uTime.value * 2.4)
          : 0
    const targetSpeed = baseSpeed + thinkingBoost
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      uBgColor: new THREE.Uniform(readBgColor()),
...
function readBgColor(): THREE.Color {
  if (typeof document === "undefined") return new THREE.Color(0x0a0a0a)
...
void main() {
    ...
    // Initialize the base color to white with zero alpha (transparent)
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (drawOval(vec2(distTheta, distRadius), vec2(0.0, 0.0), a, b, reverseGradient, softness, ovalColor)) {
            ...
            coverage = max(coverage, ovalColor.a);
        }
    }
    ...
    // In dark mode, use page background instead of pure black so dark tones blend naturally
    vec3 color1 = mix(vec3(0.0), uBgColor, uInverted);
    vec3 color2 = uColor1;
    vec3 color3 = uColor2;
    ...
    // Soft radial fade so the orb boundary is translucent, not a hard circle
    float feather = 1.0 - smoothstep(0.42, 0.92, radius);
    color.a = max(coverage, totalRingAlpha) * uOpacity * feather;
