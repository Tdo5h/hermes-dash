"use client";

import { useEffect, useState } from "react";
import { createPortal } from "react-dom";
import type { ChatSession } from "@/lib/sessions";

export type OrphanChatsPurgeDialogProps = {
  open: boolean;
  label: string;
  sessions: ChatSession[];
  onClose: () => void;
  onDeleted: () => void;
};

/**
 * Remove all chats in a sidebar “orphan” group (no manifest build id), e.g. legacy build_edit rows.
 */
export function OrphanChatsPurgeDialog({
  open,
  label,
  sessions,
  onClose,
  onDeleted,
}: OrphanChatsPurgeDialogProps) {
  const [deleting, setDeleting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [portalTarget, setPortalTarget] = useState<Element | null>(null);

  useEffect(() => {
    setPortalTarget(document.body);
  }, []);

  useEffect(() => {
    if (open) {
      setError(null);
      setDeleting(false);
    }
  }, [open, label, sessions.length]);

  if (!open || !portalTarget) return null;

  async function performDelete() {
    setDeleting(true);
    setError(null);
    try {
      for (const s of sessions) {
        const id = (s.webchatId || s.id).trim();
        if (!id) continue;
        const r = await fetch(`/api/sessions/${encodeURIComponent(id)}`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sessionKey: s.key }),
        });
        if (!r.ok) {
          const d = (await r.json().catch(() => ({}))) as { error?: string };
          throw new Error(d.error || `Could not delete session ${id}`);
        }
      }
      onDeleted();
      onClose();
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : "Could not delete");
    } finally {
      setDeleting(false);
    }
  }

  return createPortal(
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-3 sm:p-4"
      role="dialog"
      aria-modal="true"
      aria-labelledby="orphan-chats-purge-title"
      onClick={(e) => {
        if (e.target === e.currentTarget && !deleting) onClose();
      }}
    >
      <div
        className="flex max-h-[min(90dvh,28rem)] w-full max-w-sm flex-col overflow-hidden rounded-2xl border border-destructive/20 bg-[var(--sidebar-depth-canvas)] shadow-xl"
        onClick={(e) => e.stopPropagation()}
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="orphan-chats-purge-title"
        aria-describedby="orphan-chats-purge-desc"
      >
        <div className="shrink-0 border-b border-sidebar-border/30 px-4 py-3">
          <h2
            id="orphan-chats-purge-title"
            className="text-sm font-semibold text-foreground"
          >
            Remove leftover chats?
          </h2>
        </div>
        <div
          id="orphan-chats-purge-desc"
          className="min-h-0 flex-1 overflow-y-auto px-4 py-3"
        >
          <p className="text-xs leading-relaxed text-muted-foreground">
            <span className="font-medium text-foreground">{label}</span> has no
            published app in the manifest. This will permanently delete{" "}
            <span className="font-medium text-foreground">{sessions.length}</span>{" "}
            chat(s) and their messages.
          </p>
          {error ? <p className="mt-2 text-xs text-destructive/90">{error}</p> : null}
        </div>
        <div className="shrink-0 border-t border-sidebar-border/30 px-4 py-3">
          <div className="flex justify-end gap-2">
            <button
              type="button"
              onClick={onClose}
              disabled={deleting}
              className="rounded-lg px-3 py-1.5 text-xs text-muted-foreground hover:text-sidebar-foreground disabled:opacity-40"
            >
              Cancel
            </button>
            <button
              type="button"
              onClick={() => void performDelete()}
              disabled={deleting || sessions.length === 0}
              className="rounded-lg border border-destructive/50 bg-destructive/10 px-3 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/20 disabled:opacity-40"
            >
              {deleting ? "…" : "Delete chats"}
            </button>
          </div>
        </div>
      </div>
    </div>,
    portalTarget
  );
}
