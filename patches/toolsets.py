...
_HERMES_CORE_TOOLS = [
    ...
    "workspace_knowledge_write", "workspace_knowledge_read",
...
# Multi-tenant user gateways: same as core but cannot upsert
# workspace_knowledge_docs via Chat internal API (architect/automation only).
_HERMES_CLI_TENANT_CORE_TOOLS = [
    t for t in _HERMES_CORE_TOOLS if t != "workspace_knowledge_write"
]
_HERMES_CLI_TENANT_BUSINESS_CORE_TOOLS = [
    t
    for t in _HERMES_CLI_TENANT_CORE_TOOLS
    if t
    not in (
        "ha_list_entities",
        "ha_get_state",
...
# Headless shared-vault ingest worker: filesystem + shell + skills + workspace DB mirror only.
_HERMES_CLI_ARCHITECT_CORE_TOOLS = [
    "read_file",
    "write_file",
    "patch",
    "search_files",
...
TOOLSETS = {
    ...
    "hermes-cli-tenant": {
        "description": "Tenant CLI — like hermes-cli but read-only workspace knowledge API (no workspace_knowledge_write)",
        "tools": _HERMES_CLI_TENANT_CORE_TOOLS,
        "includes": []
    },
    "hermes-cli-tenant-biz": {
        "description": "Shared business / multi-tenant user gateways: hermes-cli-tenant + skills_list/skill_view/skill_manage; no ha_*; no workspace_knowledge_write",
        "tools": _HERMES_CLI_TENANT_BUSINESS_CORE_TOOLS,
        "includes": []
    },
    "hermes-cli-architect": {
        "description": "WikiVault architect only — file tools, terminal, workspace_knowledge read/write, skills read/list; no web, browser, messaging, delegation, vision, or cron",
        "tools": _HERMES_CLI_ARCHITECT_CORE_TOOLS,
        "includes": []
    },
    "hermes-cron": {
    ...
    "hermes-gateway": {
        ...
        "tools": [],
        "includes": ["hermes-telegram", "hermes-discord", "hermes-whatsapp", "hermes-slack", "hermes-signal", "hermes-bluebubbles", "hermes-email", "hermes-sms", "hermes-mattermost", "hermes-matrix", "hermes-dingtalk", "hermes-feishu", "hermes-wecom", "hermes-wecom-callback", "hermes-weixin", "hermes-qqbot", "hermes-webhook", "hermes-yuanbao"]
    }
}
...
# hermes-stack: register native HermesChat workspace knowledge tools when this
# stack bind-mounts tools/workspace_knowledge_tool.py into the gateway image.
try:
    import tools.workspace_knowledge_tool  # noqa: F401
except Exception:
    pass
try:
    import tools.image_edit_tool  # noqa: F401
except Exception:
    pass
