...
DEFAULT_HERMESCHAT_DELIVER = "webhook:http://chat:3100/api/push/send"


def _default_stack_deliver() -> str:
    """Return this stack's fallback cron delivery target.

    Chat-created jobs can arrive without platform origin metadata. Upstream
    Hermes falls back to local output in that case, which means the job runs
    but HermesChat never sees it. In this stack, no-origin scheduled jobs
    should go to the HermesChat cron webhook by default.
    """
    configured = os.getenv("HERMES_CRON_DEFAULT_DELIVER", "").strip()
    return configured or DEFAULT_HERMESCHAT_DELIVER


def _shared_owner_ids() -> tuple[int, int]:
    """Return owner uid + shared HermesChat gid for cron state.

    Cron files are written by Hermes, but HermesChat reads them for the
    Automations page. Atomic replacement creates files with the writer's
    primary group, so chmod alone is not enough; force the group back to the
    shared stack gid after every mkdir/replace.
    """
    uid_raw = os.getenv("HERMES_FS_UID", "").strip()
    gid_raw = os.getenv("HERMES_FS_GID", "").strip()
    try:
        uid = int(uid_raw) if uid_raw else HERMES_DIR.stat().st_uid
    except (OSError, ValueError):
        uid = os.getuid()
    try:
        gid = int(gid_raw) if gid_raw else HERMES_DIR.stat().st_gid
    except (OSError, ValueError):
        gid = os.getgid()
    return uid, gid


def _chown_shared(path: Path) -> None:
    uid, gid = _shared_owner_ids()
    try:
...
def _secure_dir(path: Path):
    """Set cron dirs to Hermes owner + HermesChat group access.

    Upstream Hermes uses 0700/0600 because it assumes one local user. In this
    stack Hermes writes cron jobs, while HermesChat reads them for the
    Automations page. Keep the directory setgid so rewritten temp files inherit
    the shared group after the entrypoint has normalized ownership.
    """
    _chown_shared(path)
    try:
...
def _secure_file(path: Path):
    """Set cron files readable by HermesChat's shared group."""
    if not path.exists():
        return
    _chown_shared(path)
    try:
...
def create_job(
    ...
    # Default delivery to origin if available, otherwise the HermesChat webhook.
    # Normal HermesChat API sessions may not expose origin metadata to cronjob(),
    # so falling back to local would make successful jobs invisible in chat.
    if deliver is None:
        deliver = "origin" if origin else _default_stack_deliver()
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for i, job in enumerate(jobs):
        ...
        if updates.get("deliver") == "origin" and not job.get("origin"):
            updates["deliver"] = _default_stack_deliver()
