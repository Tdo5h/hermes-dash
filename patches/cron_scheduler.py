...
def _cron_post_hermeschat_webhook(url: str, job: dict, content: str) -> Optional[str]:
    """POST JSON to HermesChat `/api/push/send` or a compatible webhook."""
    import urllib.error
    import urllib.request
    token = os.environ.get("PUSH_WEBHOOK_TOKEN", "").strip()
    payload: dict = {
        "jobId": job.get("id", ""),
    ...
    try:
        with urllib.request.urlopen(req, timeout=45) as resp:
            if resp.status != 200:
                ...
                return f"webhook HTTP {resp.status}: {body[:300]}"
    except urllib.error.HTTPError as e:
        err_body = e.read().decode("utf-8", errors="replace") if e.fp else ""
        return f"webhook HTTP {e.code}: {err_body[:300]}"
    except Exception as e:
        return f"webhook POST failed: {e}"
    return None
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        # hermes-stack: outbound HTTP delivery for HermesChat reminders.
        # Example deliver value: webhook:http://chat:3100/api/push/send
        if str(platform_name).lower() == "webhook":
            err = _cron_post_hermeschat_webhook(str(chat_id), job, cleaned_delivery_content)
            if err:
                logger.warning("Job '%s': %s", job["id"], err)
                delivery_errors.append(err)
            else:
                logger.info("Job '%s': delivered to webhook %s", job["id"], chat_id)
            continue
