...
# Models can paste OpenRouter/Gemini data URLs into write_file. Decode those
# into real bytes for image paths instead of storing the ASCII data URI.
_DATA_URI_WRITE_RE = re.compile(
    r"^data:(image/[a-zA-Z0-9.+-]+);base64,([A-Za-z0-9+/=\s]+)\s*$",
    re.IGNORECASE | re.DOTALL,
)
_MAX_DATA_URI_WRITE_CHARS = 40_000_000
_MAX_DECODED_IMAGE_WRITE_BYTES = 35 * 1024 * 1024


def _try_decode_image_data_uri_for_write(path: str, content: str) -> Optional[bytes]:
    """If path looks like an image and content is a single data URL, return raw bytes."""
    try:
        ext = os.path.splitext(path)[1].lower()
        if ext not in IMAGE_EXTENSIONS:
            return None
        s = (content or "").strip()
        if len(s) > _MAX_DATA_URI_WRITE_CHARS:
            return None
        if not s.startswith("data:image/"):
            return None
        match = _DATA_URI_WRITE_RE.match(s)
        if not match:
            return None
        b64 = re.sub(r"\s+", "", match.group(2))
        raw = base64.standard_b64decode(b64, validate=False)
        if len(raw) > _MAX_DECODED_IMAGE_WRITE_BYTES:
            return None
        return raw
    except Exception:
        return None
...
# HermesChat runs as nextjs (gid 1001); gateway writes builds as hermes:1001.
# Default umask yields dirs without group write, so deleting Builds from Chat can fail.
_BUILDS_GATEWAY_ROOT = "/opt/data/builds"


def _relax_builds_permissions_for_hermes_chat(path: str) -> None:
    """chmod g+w on files and setgid dirs under the builds tree so Chat can manage apps."""
    try:
        abs_path = os.path.realpath(path)
        builds_root = os.path.realpath(_BUILDS_GATEWAY_ROOT)
    except OSError:
        return
    if abs_path != builds_root and not abs_path.startswith(builds_root + os.sep):
        return
    try:
        if os.path.isfile(abs_path):
            os.chmod(abs_path, 0o664)
        elif os.path.isdir(abs_path):
            os.chmod(abs_path, 0o2775)
    except OSError:
        pass
    d = os.path.dirname(abs_path) if os.path.isfile(abs_path) else abs_path
    while d and d.startswith(builds_root):
        try:
            os.chmod(d, 0o2775)
        except OSError:
            pass
...
class ShellFileOperations(FileOperations):
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        raw_image = _try_decode_image_data_uri_for_write(path, content)
        if raw_image is not None:
            # Pipe base64 to base64 -d so binary lands on disk; cat/stdin text
            # paths can corrupt NUL bytes depending on backend adapters.
            b64_in = base64.standard_b64encode(raw_image).decode("ascii")
            write_cmd = f"base64 -d > {self._escape_shell_arg(path)}"
            write_result = self._exec(write_cmd, stdin_data=b64_in)
            if write_result.exit_code != 0:
                return WriteResult(error=f"Failed to write file: {write_result.stdout}")
            _relax_builds_permissions_for_hermes_chat(path)
            return WriteResult(
                bytes_written=len(raw_image),
