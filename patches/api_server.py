...
class APIServerAdapter(BasePlatformAdapter):
    ...
    @staticmethod
    def _normalize_api_request_model(raw: Optional[Any]) -> Optional[str]:
        """Map OpenAI body `model` to a concrete provider catalog id."""
        if raw is None:
            return None
        value = str(raw).strip()
        if not value:
            return None
        if value.lower() in ("hermes-agent", "openclaw"):
            return None
        return value

    @staticmethod
    def _api_reasoning_config_from_body(body: Dict[str, Any]) -> tuple[bool, Optional[Dict[str, Any]]]:
        raw: Any = None
        if "reasoning_effort" in body:
            raw = body.get("reasoning_effort")
        elif isinstance(body.get("reasoning"), dict):
            raw = body.get("reasoning", {}).get("effort")
        elif "reasoningEffort" in body:
            raw = body.get("reasoningEffort")
        if raw is None:
            return False, None
        effort = str(raw or "").strip().lower()
        try:
            from hermes_constants import parse_reasoning_effort
            return True, parse_reasoning_effort(effort)
        except Exception:
            if effort == "none":
                return True, {"enabled": False}
            if effort in ("minimal", "low", "medium", "high", "xhigh"):
                return True, {"enabled": True, "effort": effort}
        return True, None

    @staticmethod
    def _api_service_tier_from_body(body: Dict[str, Any]) -> tuple[bool, Optional[str]]:
        if "service_tier" in body:
            raw = body.get("service_tier")
        elif "serviceTier" in body:
            raw = body.get("serviceTier")
        else:
            return False, None
        value = str(raw or "").strip().lower()
        if value in ("fast", "priority", "on"):
            return True, "priority"
        if value in ("normal", "standard", "default", "off", "none", ""):
            return True, None
        logger.warning("Unknown request service_tier '%s', ignoring", raw)
        return True, None

    @staticmethod
    def _sanitize_request_overrides(
        overrides: Dict[str, Any],
        *,
        model: Optional[str] = None,
        runtime_kwargs: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """Drop request fields rejected by provider-specific backends."""
        cleaned = dict(overrides or {})
        runtime = runtime_kwargs or {}
        provider = str(runtime.get("provider") or "").strip().lower()
        base_url = str(runtime.get("base_url") or "").strip().lower()
        is_codex = (
            provider in ("openai-codex", "codex", "chatgpt")
            or "chatgpt.com/backend-api/codex" in base_url
        )
        if is_codex:
            # The ChatGPT Codex Responses bridge rejects OpenAI chat-style
            # sampling/output-limit fields. Leaving these in makes Create runs
            # fail before the model starts streaming.
            for key in (
                "max_tokens",
    ...
    def _create_agent(
        ...
        use_runtime_reasoning_config: bool = True,
        use_runtime_service_tier: bool = True,
    ) -> Any:
        """
        ...
        override = self._normalize_api_request_model(request_model)
        if override:
            model = override
        ...
        if effective_reasoning_config is None and use_runtime_reasoning_config:
            try:
                effective_reasoning_config = GatewayRunner._load_reasoning_config()
            except Exception:
                effective_reasoning_config = None
        effective_service_tier = service_tier
        if effective_service_tier is None and use_runtime_service_tier:
            try:
                effective_service_tier = GatewayRunner._load_service_tier()
            except Exception:
        ...
        if effective_service_tier:
            try:
                ...
                pass
        try:
            raw_overrides = ((user_config or {}).get("agent") or {}).get("request_overrides")
            if isinstance(raw_overrides, dict):
                request_overrides.update(raw_overrides)
        except Exception:
            pass
        request_overrides = self._sanitize_request_overrides(
            request_overrides,
            model=model,
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        created = int(time.time())
        reasoning_provided, request_reasoning_config = self._api_reasoning_config_from_body(body)
        service_tier_provided, request_service_tier = self._api_service_tier_from_body(body)
        ...
        if stream:
            ...
            agent_task = asyncio.ensure_future(self._run_agent(
                ...
                use_runtime_reasoning_config=not reasoning_provided,
                use_runtime_service_tier=not service_tier_provided,
            ))
        ...
        if idempotency_key:
            fp = _make_request_fingerprint(
                body,
                keys=["model", "messages", "tools", "tool_choice", "stream", "reasoning", "reasoning_effort", "service_tier"],
            )
            try:
    ...
    # ------------------------------------------------------------------
    # Stack model config + presets API
    # ------------------------------------------------------------------

    _STACK_PRESETS_NAME = "model_presets.json"
    _STACK_INGEST_FILE = "ingest_chat_model"
    _STACK_TITLE_FILE = "title_chat_model"
    _STACK_VALIDATOR_MODEL_FILE = "validator_chat_model"
    _STACK_VALIDATOR_ENABLED_FILE = "validator_enabled"
    _STACK_CODEX_BASE_URL = "https://chatgpt.com/backend-api/codex"
    _STACK_MAX_STR = 512

    @classmethod
    def _stack_hermes_home(cls) -> Path:
        from hermes_cli.config import get_hermes_home

        return Path(get_hermes_home())

    @classmethod
    def _stack_atomic_write_bytes(cls, path: Path, data: bytes) -> None:
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                gid_raw = os.getenv("HERMES_FS_GID", "1001").strip()
                os.chown(path, -1, int(gid_raw) if gid_raw else 1001)
    ...
    @classmethod
    def _stack_read_config_dict(cls) -> Dict[str, Any]:
        if yaml is None:
            raise RuntimeError("PyYAML is not available")
        p = cls._stack_hermes_home() / "config.yaml"
        if not p.is_file():
            return {}
    ...
    @classmethod
    def _stack_write_config_dict(cls, doc: Dict[str, Any]) -> None:
        if yaml is None:
            raise RuntimeError("PyYAML is not available")
        out = yaml.safe_dump(doc, default_flow_style=False, allow_unicode=True, sort_keys=False)
        cls._stack_atomic_write_bytes(cls._stack_hermes_home() / "config.yaml", out.encode("utf-8"))

    @classmethod
    def _stack_load_presets(cls, doc: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        p = cls._stack_hermes_home() / cls._STACK_PRESETS_NAME
        if p.is_file():
            try:
        ...
        if isinstance(model, dict):
            ...
            main_provider = str(model.get("provider") or "openrouter").strip() or "openrouter"
            main_base_url = str(model.get("base_url") or "").strip()
        else:
            main_model = str(model or "").strip()
        ...
        base = {
            ...
            "ingestModel": main_model,
            "ingestProvider": main_provider,
            "fallbackModel": "",
            "fallbackProvider": "openrouter",
            "titleModel": "",
            "titleProvider": "openrouter",
            "imageGenModel": "",
        ...
        return {
            "active": "balanced",
            "presets": {
                "frontier": {**base, "label": "Max Frontier"},
                "balanced": base,
                "economy": {**base, "label": "Economy"},
                "codex": {
                    **base,
                    "label": "ChatGPT",
                    "mainProvider": "openai-codex",
                    "mainBaseUrl": cls._STACK_CODEX_BASE_URL,
                    "ingestProvider": "openai-codex",
                    "fallbackProvider": "openai-codex",
                    "titleProvider": "openai-codex",
                    "imageGenProvider": "openai-codex",
                    "imageGenBaseUrl": cls._STACK_CODEX_BASE_URL,
                    "cheapProvider": "openai-codex",
                    "serviceTier": "fast",
                },
            },
        }
    ...
    @classmethod
    def _stack_write_presets(cls, presets: Dict[str, Any]) -> None:
        cls._stack_atomic_write_bytes(
            cls._stack_hermes_home() / cls._STACK_PRESETS_NAME,
            json.dumps(presets, indent=2, ensure_ascii=False).encode("utf-8"),
        )

    @classmethod
    def _stack_write_or_remove_line_file(cls, filename: str, value: Any) -> None:
        p = cls._stack_hermes_home() / filename
        text = str(value or "").strip()
        if text:
            cls._stack_atomic_write_bytes(p, (text + "\n").encode("utf-8"))
            return
        try:
            if p.is_file():
                p.unlink()
    ...
    @staticmethod
    def _stack_str(value: Any, default: str = "") -> str:
        return str(value if value is not None else default).strip()

    @classmethod
    def _stack_apply_bundle_to_config(cls, doc: Dict[str, Any], bundle: Dict[str, Any]) -> None:
        model_doc = doc.get("model")
        if not isinstance(model_doc, dict):
            model_doc = {}
        main_model = cls._stack_str(bundle.get("mainModel"))
        main_provider = cls._stack_str(bundle.get("mainProvider"), "openrouter") or "openrouter"
        main_base = cls._stack_str(bundle.get("mainBaseUrl"))
        model_doc["default"] = main_model
        model_doc["provider"] = main_provider
        if main_base:
            model_doc["base_url"] = main_base
        elif main_provider == "openai-codex":
            model_doc["base_url"] = cls._STACK_CODEX_BASE_URL
        elif main_provider == "openrouter":
            model_doc["base_url"] = "https://openrouter.ai/api/v1"
        else:
            model_doc.pop("base_url", None)
        doc["model"] = model_doc

        fallback_model = cls._stack_str(bundle.get("fallbackModel"))
        fallback_provider = cls._stack_str(bundle.get("fallbackProvider"), "openrouter") or "openrouter"
        fallback_base = cls._stack_str(bundle.get("fallbackBaseUrl"))
        if fallback_model or fallback_provider:
            fb: Dict[str, Any] = {"model": fallback_model, "provider": fallback_provider}
            if fallback_base:
                fb["base_url"] = fallback_base
            elif fallback_provider == "openai-codex":
                fb["base_url"] = cls._STACK_CODEX_BASE_URL
            doc["fallback_model"] = fb
        else:
            doc.pop("fallback_model", None)

        agent_doc = doc.get("agent")
        if not isinstance(agent_doc, dict):
            agent_doc = {}
        effort = cls._stack_str(bundle.get("reasoningEffort")).lower()
        if effort in {"low", "medium", "high", "xhigh"}:
            agent_doc["reasoning_effort"] = effort
        tier = cls._stack_str(bundle.get("serviceTier")).lower()
        if tier:
            agent_doc["service_tier"] = "fast" if tier in {"fast", "priority", "on"} else "normal"
        doc["agent"] = agent_doc

        image_doc = doc.get("image_gen")
        if not isinstance(image_doc, dict):
            image_doc = {}
        if "imageGenModel" in bundle:
            image_model = cls._stack_str(bundle.get("imageGenModel"))
            if image_model:
                image_doc["model"] = image_model
            else:
                image_doc.pop("model", None)
        if "imageGenProvider" in bundle:
            image_provider = cls._stack_str(bundle.get("imageGenProvider"))
            if image_provider:
                image_doc["provider"] = image_provider
            else:
                image_doc.pop("provider", None)
        if "imageGenBaseUrl" in bundle:
            image_base = cls._stack_str(bundle.get("imageGenBaseUrl"))
            if image_base:
                image_doc["base_url"] = image_base
            elif image_doc.get("provider") == "openai-codex":
                image_doc["base_url"] = cls._STACK_CODEX_BASE_URL
            else:
                image_doc.pop("base_url", None)
        image_doc["use_gateway"] = False
        doc["image_gen"] = image_doc

        routing_doc = doc.get("smart_model_routing")
        if not isinstance(routing_doc, dict):
            routing_doc = {}
        if isinstance(bundle.get("smartRoutingEnabled"), bool):
            routing_doc["enabled"] = bool(bundle.get("smartRoutingEnabled"))
        if "cheapModel" in bundle or "cheapProvider" in bundle:
            cheap_doc = routing_doc.get("cheap_model")
            if not isinstance(cheap_doc, dict):
                cheap_doc = {}
            cheap_doc["model"] = cls._stack_str(bundle.get("cheapModel"))
            cheap_doc["provider"] = cls._stack_str(bundle.get("cheapProvider"), "openrouter") or "openrouter"
            cheap_base = cls._stack_str(bundle.get("cheapBaseUrl"))
            if cheap_base:
                cheap_doc["base_url"] = cheap_base
            elif cheap_doc["provider"] == "openai-codex":
                cheap_doc["base_url"] = cls._STACK_CODEX_BASE_URL
            else:
                cheap_doc.pop("base_url", None)
            routing_doc["cheap_model"] = cheap_doc
        doc["smart_model_routing"] = routing_doc

        if isinstance(bundle.get("auxiliary"), dict):
            doc["auxiliary"] = bundle["auxiliary"]

        cls._stack_write_or_remove_line_file(cls._STACK_INGEST_FILE, bundle.get("ingestModel"))
        if "titleModel" in bundle:
            cls._stack_write_or_remove_line_file(cls._STACK_TITLE_FILE, bundle.get("titleModel"))
        if "validatorModel" in bundle:
            cls._stack_write_or_remove_line_file(
                cls._STACK_VALIDATOR_MODEL_FILE,
                bundle.get("validatorModel"),
            )
        if isinstance(bundle.get("validatorEnabled"), bool):
            cls._stack_write_or_remove_line_file(
                cls._STACK_VALIDATOR_ENABLED_FILE,
                "true" if bundle.get("validatorEnabled") else "false",
            )

    @staticmethod
    def _stack_request_soft_reload() -> None:
        os.kill(os.getpid(), signal.SIGUSR1)

    async def _handle_stack_model_state_get(self, request: "web.Request") -> "web.Response":
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        if yaml is None:
            return web.json_response({"error": "PyYAML not available"}, status=501)
        try:
            doc = self._stack_read_config_dict()
            presets = self._stack_load_presets(doc)
            jobs_out: List[Dict[str, Any]] = []
            if _CRON_AVAILABLE and _cron_list is not None:
                try:
                    jobs_out = _cron_list(include_disabled=True)
                except Exception as e:
                    logger.debug("stack model-state jobs list: %s", e)
            return web.json_response({
                "ok": True,
                "config": doc,
                "presets": presets,
    ...
    async def _handle_stack_model_state_post(self, request: "web.Request") -> "web.Response":
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        if yaml is None:
        ...
        try:
            doc = self._stack_read_config_dict()
            presets = self._stack_load_presets(doc)
            if "active" in body and isinstance(body.get("active"), str):
                presets["active"] = str(body["active"]).strip()
            if "presets" in body:
                raw_presets = body["presets"]
                if not isinstance(raw_presets, dict):
                    return web.json_response({"error": "presets must be an object"}, status=400)
                current = presets.setdefault("presets", {})
                ...
                for key, value in raw_presets.items():
                    ...
                    for field, field_value in value.items():
                        ...
                        if isinstance(field_value, bool):
                            clean_bundle[field] = field_value
                        elif isinstance(field_value, dict):
                            if len(json.dumps(field_value, ensure_ascii=False)) <= 200_000:
                                clean_bundle[field] = field_value
                        elif isinstance(field_value, str) and len(field_value) <= self._STACK_MAX_STR:
                            clean_bundle[field] = field_value
                    current[key] = {**(current.get(key) if isinstance(current.get(key), dict) else {}), **clean_bundle}
                self._stack_write_presets(presets)

            patch = body.get("patchConfig")
            if isinstance(patch, dict):
                ...
                if isinstance(model_patch, dict):
                    ...
                    if not isinstance(model_doc, dict):
                        model_doc = {}
                    if isinstance(model_patch.get("default"), str):
                        model_doc["default"] = model_patch["default"].strip()[: self._STACK_MAX_STR]
                    if isinstance(model_patch.get("provider"), str):
                        model_doc["provider"] = model_patch["provider"].strip()[: self._STACK_MAX_STR]
                    doc["model"] = model_doc
                fallback_patch = patch.get("fallback_model")
                if isinstance(fallback_patch, dict):
                    fb_model = fallback_patch.get("model")
                    fb_provider = fallback_patch.get("provider")
                    if fb_model or fb_provider:
                        doc["fallback_model"] = {
                            "model": str(fb_model or "").strip()[: self._STACK_MAX_STR],
                            "provider": str(fb_provider or "openrouter").strip()[: self._STACK_MAX_STR] or "openrouter",
                        }
                    else:
                        doc.pop("fallback_model", None)

            if body.get("applyActivePreset") is True:
                merged = self._stack_load_presets(doc)
                active = str(merged.get("active") or "balanced")
                bundles = merged.get("presets") if isinstance(merged.get("presets"), dict) else {}
                bundle = bundles.get(active) if isinstance(bundles, dict) else None
                if not isinstance(bundle, dict):
                    return web.json_response({"error": f"Unknown active preset: {active}"}, status=400)
                self._stack_apply_bundle_to_config(doc, bundle)

            if patch is not None or body.get("applyActivePreset") is True:
                self._stack_write_config_dict(doc)

            reload_after = bool(body.get("reload", True) or body.get("reloadOnly") is True)
            if reload_after:
                try:
                    self._stack_request_soft_reload()
                except OSError as e:
                    return web.json_response({"ok": True, "warning": f"config saved but reload signal failed: {e}"})
            return web.json_response({"ok": True, "reloaded": reload_after})
        except Exception as e:
            logger.exception("Stack model-state update failed")
            return web.json_response({"error": str(e)}, status=500)
    ...
    @staticmethod
    def _kanban_task_to_dict(task: Any, latest_summary: Optional[str] = None) -> Dict[str, Any]:
        data = {
            "id": task.id,
            "title": task.title,
            "body": task.body,
            ...
            "last_failure_error": getattr(task, "last_failure_error", None),
    ...
    @staticmethod
    def _kanban_event_to_dict(event: Any) -> Dict[str, Any]:
        return {
            "id": event.id,
    ...
    @staticmethod
    def _kanban_str_list(raw: Any) -> List[str]:
        if raw is None:
            return []
    ...
    async def _handle_kanban_list_boards(self, request: "web.Request") -> "web.Response":
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        try:
            ...
            logger.exception("Kanban list boards failed")
            return web.json_response({"error": str(e)}, status=500)

    async def _handle_kanban_create_board(self, request: "web.Request") -> "web.Response":
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        try:
        ...
        try:
            ...
            logger.exception("Kanban create board failed")
            return web.json_response({"error": str(e)}, status=500)

    async def _handle_kanban_delete_board(self, request: "web.Request") -> "web.Response":
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        board = str(request.match_info.get("board", "") or "").strip()
        if not board:
            return web.json_response({"error": "board required"}, status=400)
        hard_delete = request.query.get("delete") in {"1", "true", "yes"}
        try:
            from hermes_cli import kanban_db as kb

            result = kb.remove_board(board, archive=not hard_delete)
            return web.json_response({
                "board": board,
                "result": result,
                "deleted": bool(hard_delete),
                "archived": not bool(hard_delete),
            })
        except ValueError as e:
            return web.json_response({"error": str(e)}, status=400)
        except Exception as e:
            logger.exception("Kanban delete board failed")
            return web.json_response({"error": str(e)}, status=500)
    ...
    async def _handle_kanban_list_tasks(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            try:
                summary_by_id = kb.latest_summaries(conn, [t.id for t in tasks])
            except Exception:
                summary_by_id = {}
            return web.json_response({
                "board": board,
                "tasks": [
                    self._kanban_task_to_dict(task, summary_by_id.get(task.id))
                    for task in tasks
                ],
            })
        except ValueError as e:
            ...
            logger.exception("Kanban list tasks failed")
            return web.json_response({"error": str(e)}, status=500)
        finally:
            if conn is not None:
                conn.close()
    ...
    async def _handle_kanban_create_task(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            task_id = kb.create_task(
                conn,
                title=title,
                body=str(body.get("body")) if body.get("body") is not None else None,
                assignee=str(body.get("assignee")).strip() if body.get("assignee") else None,
                created_by=str(body.get("createdBy") or body.get("created_by") or "hermes-chat"),
                workspace_kind=str(body.get("workspaceKind") or body.get("workspace_kind") or "scratch"),
                workspace_path=(
                    str(body.get("workspacePath") or body.get("workspace_path")).strip()
                    if body.get("workspacePath") or body.get("workspace_path")
                    else None
                ),
                tenant=str(body.get("tenant")).strip() if body.get("tenant") else None,
                priority=int(body.get("priority") or 0),
                parents=self._kanban_str_list(body.get("parents") or body.get("parent")),
                triage=bool(body.get("triage", False)),
                idempotency_key=(
                    str(body.get("idempotencyKey") or body.get("idempotency_key")).strip()
                    if body.get("idempotencyKey") or body.get("idempotency_key")
                    else None
                ),
                max_runtime_seconds=(
                    int(body.get("maxRuntimeSeconds") or body.get("max_runtime_seconds"))
                    if body.get("maxRuntimeSeconds") or body.get("max_runtime_seconds")
                    else None
                ),
                skills=self._kanban_str_list(body.get("skills") or body.get("skill")),
                max_retries=(
                    int(body.get("maxRetries") or body.get("max_retries"))
                    if body.get("maxRetries") or body.get("max_retries")
                    else None
                ),
            )
            ...
            return web.json_response({
                "board": board,
                "task": self._kanban_task_to_dict(task) if task else {"id": task_id},
            })
        except ValueError as e:
            return web.json_response({"error": str(e)}, status=400)
        except Exception as e:
            logger.exception("Kanban create task failed")
            return web.json_response({"error": str(e)}, status=500)
        finally:
            if conn is not None:
                conn.close()
    ...
    async def _handle_kanban_get_task(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            summary = None
            try:
                summary = kb.latest_summary(conn, task.id)
            except Exception:
                summary = None
            return web.json_response({
                "board": board,
                "task": self._kanban_task_to_dict(task, summary),
            })
        except ValueError as e:
            return web.json_response({"error": str(e)}, status=400)
        except Exception as e:
            logger.exception("Kanban get task failed")
            return web.json_response({"error": str(e)}, status=500)
        finally:
            if conn is not None:
                conn.close()

    async def _handle_kanban_task_events(self, request: "web.Request") -> "web.Response":
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        board = request.match_info.get("board", "")
        ...
        try:
            ...
            return web.json_response({
                "board": board,
                "taskId": task_id,
                "events": [self._kanban_event_to_dict(event) for event in events],
            })
        except ValueError as e:
            return web.json_response({"error": str(e)}, status=400)
        except Exception as e:
            logger.exception("Kanban task events failed")
            return web.json_response({"error": str(e)}, status=500)
        finally:
            if conn is not None:
                conn.close()
    ...
    async def _run_agent(
        ...
        def _run():
            agent = self._create_agent(
                ...
                use_runtime_reasoning_config=use_runtime_reasoning_config,
                use_runtime_service_tier=use_runtime_service_tier,
            )
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # HermesChat stack model/preset API
            self._app.router.add_get("/api/stack/model-state", self._handle_stack_model_state_get)
            self._app.router.add_post("/api/stack/model-state", self._handle_stack_model_state_post)
            # Kanban board API for authenticated HermesChat orchestration/read-only feeds
            ...
            self._app.router.add_delete("/api/kanban/boards/{board}", self._handle_kanban_delete_board)
            self._app.router.add_get("/api/kanban/boards/{board}/tasks", self._handle_kanban_list_tasks)
            self._app.router.add_post("/api/kanban/boards/{board}/tasks", self._handle_kanban_create_task)
            self._app.router.add_get("/api/kanban/boards/{board}/tasks/{task_id}", self._handle_kanban_get_task)
            self._app.router.add_get("/api/kanban/boards/{board}/tasks/{task_id}/events", self._handle_kanban_task_events)
