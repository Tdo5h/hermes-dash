...
def _shared_owner_ids() -> tuple[int, int]:
    ...
    try:
        uid = int(uid_raw) if uid_raw else home.stat().st_uid
    except (OSError, ValueError):
        uid = os.getuid()
    try:
        gid = int(gid_raw) if gid_raw else home.stat().st_gid
    except (OSError, ValueError):
        gid = os.getgid()
    return uid, gid


def _secure_shared_file(path: Path) -> None:
    """Keep telemetry writable by Hermes and HermesChat after atomic replace."""
